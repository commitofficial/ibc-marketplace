...
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Long: fmt.Sprintf(`This command is an alias of the 'releyard tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate, defaultMinSelfDelegation),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = app.RelayerValidateGenesisState(genesisState); err != nil {
				return err
			}
			...
			if info.GetType() == kbkeys.TypeOffline || info.GetType() == kbkeys.TypeMulti {
				fmt.Println("Offline key passed in. Use `relayercli tx sign` command to sign:")
