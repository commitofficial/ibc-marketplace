...
import (
	...
	ibc2 "github.com/ibc-marketplace/modules/ibc"
)
...
func (c relayCommander) refine(bz []byte, ibcSeq, accSeq uint64, passphrase string) []byte {
	var msg ctypes.Msg
	if err := c.cdc.UnmarshalBinaryLengthPrefixed(bz, &msg); err != nil {
		panic(err)
	}
	
	switch msg := msg.(type) {
	case ibc2.MsgIBCExchangeOrder:
		
		relayMsg := ibc2.NewMsgIBCExchangeOrder(msg.IBCPacket, c.address, accSeq)
		txBldr := authtxb.NewTxBuilderFromCLI().WithSequence(accSeq).WithTxEncoder(utils.GetTxEncoder(c.cdc))
		cliCtx := context.NewCLIContext()
		
		name := cliCtx.GetFromName()
		res, err := txBldr.BuildAndSign(name, passphrase, []ctypes.Msg{relayMsg})
		if err != nil {
			panic(err)
		}
